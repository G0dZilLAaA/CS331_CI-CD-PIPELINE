"""
JavaScript/TypeScript Mutation Operators — tree-sitter + string surgery.

Same mutation concepts as Python operators:
    - Relational swap
    - Arithmetic swap
    - Conditional negate
    - Boundary mutate
...
class JS_Relational_Swap(TS_Operator_Base):
    name = "relational_swap"
    description = "Swaps relational comparison operators (JS/TS)"
...
class JS_Arithmetic_Swap(TS_Operator_Base):
    name = "arithmetic_swap"
    description = "Swaps arithmetic operators (JS/TS)"
...
class JS_Conditional_Negate(TS_Operator_Base):
    name = "conditional_negate"
    description = "Negates if-statement conditions (JS/TS)"
            ...
            if node.type == "if_statement":
                for child in node.children:
                    if child.type == "parenthesized_expression":
                        # Get the inner expression (strip outer parens)
...
class JS_Boundary_Mutate(TS_Operator_Base):
    name = "boundary_mutate"
    description = "Shifts integer constants by ±1 (JS/TS)"
...
class JS_Return_Mutate(TS_Operator_Base):
    name = "return_mutate"
    description = "Replaces return values with undefined (JS/TS)"

    def find_targets(self, tree, source_bytes):
        targets = []
        for node in self._walk(tree.root_node):
            if node.type == "return_statement":
                # Get everything after 'return' keyword
    ...
    def apply(self, source_bytes, target):
        return self._replace_bytes(
            source_bytes, target["start_byte"], target["end_byte"], "undefined"
        )

    def describe_mutation(self, target):
        return f"Line {target['line']}: Changed return value to undefined"


ACTIVE_OPERATORS_JS = [
    JS_Relational_Swap(),
    JS_Arithmetic_Swap(),
    JS_Conditional_Negate(),
    JS_Boundary_Mutate(),
    JS_Return_Mutate(),
]