"""
Java Mutation Operators — tree-sitter + string surgery.

Same mutation concepts, Java-specific node types.
"""
...
class Java_Relational_Swap(TS_Operator_Base):
    name = "relational_swap"
    description = "Swaps relational comparison operators (Java)"
...
class Java_Arithmetic_Swap(TS_Operator_Base):
    name = "arithmetic_swap"
    description = "Swaps arithmetic operators (Java)"
...
class Java_Conditional_Negate(TS_Operator_Base):
    name = "conditional_negate"
    description = "Negates if-statement conditions (Java)"
...
class Java_Boundary_Mutate(TS_Operator_Base):
    name = "boundary_mutate"
    description = "Shifts integer constants by ±1 (Java)"

    def find_targets(self, tree, source_bytes):
        targets = []
        for node in self._walk(tree.root_node):
            if node.type in ("decimal_integer_literal", "integer_literal"):
...
class Java_Return_Mutate(TS_Operator_Base):
    name = "return_mutate"
    description = "Replaces return values with null (Java)"
            ...
            if node.type == "return_statement":
                ...
                if children:
                    ...
                    targets.append({
                        ...
                        "original": self._node_text(val_node, source_bytes),
                        "line": node.start_point[0] + 1
                    })
        return targets
    ...
    def apply(self, source_bytes, target):
        return self._replace_bytes(
            source_bytes, target["start_byte"], target["end_byte"], "null"
        )

    def describe_mutation(self, target):
        return f"Line {target['line']}: Changed return value to null"


ACTIVE_OPERATORS_JAVA = [
    Java_Relational_Swap(),
    Java_Arithmetic_Swap(),
    Java_Conditional_Negate(),
    Java_Boundary_Mutate(),
    Java_Return_Mutate(),
]